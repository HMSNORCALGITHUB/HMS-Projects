...
def plot_by_date_bar(df, column, title=None):
    ...
    ab_counts = df[column].str.contains("AB").sum()
    
    sb_counts = df[column].str.contains("SB").sum()
...
def plot_by_number_hist(df, column, title=None, top_n=None):
    import matplotlib.pyplot as plt
    # makes dataframe of all values in a column, drops NaN
    data = df[column].dropna()
    # min value is int value of the column's minimum
    min_val = int(data.min())
    max_val = int(data.max())
    bin_edges = range((min_val % 10), max_val + 10, 10)

    fig, ax = plt.subplots(figsize=(6, 6))
    ax.hist(data, bins=bin_edges, color="#4C72B0", edgecolor="white")
...
def ab_vs_sb(df, column, title=None):
    import matplotlib.pyplot as plt
    ab_counts = df[column].str.contains("AB", na=False).sum()
        
    sb_counts = df[column].str.contains("SB", na=False).sum()
    roomlist = ['ab',
            'sb']
    countlist = [ab_counts, sb_counts]
    fig, ax = plt.subplots(figsize=(8, 5))
    bars = ax.bar(roomlist, countlist, color="#4C72B0")
    ax.set_ylim(0, max(countlist) * 1.40)
    # fmt is a format package, %d converts a number to an integer
    ax.bar_label(bars, fmt="%d")
    ax.set_ylabel("Count")
    ax.set_xlabel("Room")
    ...
    plt.tight_layout()
    plt.subplots_adjust(top=0.85)
    plt.title(f"{column} Room Distribution")
    plt.show()



def plot_percent_hist(df, column, title=None):
    import matplotlib.pyplot as plt
    from matplotlib.ticker import PercentFormatter

    data = df[column].dropna()

    bin_edges = [i / 10 for i in range(0, 11)]  # 0.0, 0.1, 0.2, ..., 1.0
